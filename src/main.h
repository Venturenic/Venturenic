...
static const int MODIFIER_INTERVAL_SWITCH = 3500;
...
public:
    ...
    {
        // Venturenic: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // Venturenic: get transaction coin age
...
public:
    ...
    // Venturenic: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // Venturenic: two types of block: proof-of-work or proof-of-stake
    ...
    // Venturenic: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // Venturenic: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // Venturenic: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // Venturenic: block index flags
    enum  
    {
