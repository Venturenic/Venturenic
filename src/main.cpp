...
static const int64_t nTargetTimespan_legacy = nTargetSpacing * nRetarget; // nTargetTimespan_legacy = 1200
static const int64_t nInterval = nTargetTimespan_legacy / nTargetSpacing; // nInterval = 20
...
const string strMessageMagic = "Venturenic Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Venturenic: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Venturenic: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Venturenic: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Venturenic: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Venturenic: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Venturenic: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Venturenic: track money supply and mint amount info
...
// Venturenic: total coin age spent in transaction, in the unit of coin-days.
...
// Venturenic: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // Venturenic: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Venturenic: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Venturenic: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // Venturenic: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // Venturenic: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Venturenic: check proof-of-stake
    ...
    // Venturenic: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Venturenic: getblocks may not obtain the ancestor block rejected
    ...
    // Venturenic: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Venturenic", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Ventureniccoin";
        CTransaction txNew;
        txNew.nTime = 1427810005;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x4384fb0f77d0c1a614be76794ffa46339741e59e3137e8186d0700c409b48436"));
        block.print();
        ...
        // Venturenic: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x5a, 0x6f, 0x11, 0xfa };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // Venturenic: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Venturenic: tell downloading node about the latest block if it's
